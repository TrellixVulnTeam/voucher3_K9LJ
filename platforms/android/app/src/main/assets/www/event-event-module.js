...
let EventPage = class EventPage {
    ...
    ionViewWillEnter() {
        this.event = this.activatedRoute.snapshot.paramMap.get('event');
        this.user = this.activatedRoute.snapshot.paramMap.get('user');
        firebase_app__WEBPACK_IMPORTED_MODULE_6__["default"].database().ref('events/' + this.event).once('value', data => {
            if (data.exists()) {
                ...
                for (const key in (data.val()['vouchers'] || {})) {
                    ...
                    firebase_app__WEBPACK_IMPORTED_MODULE_6__["default"].database().ref('vouchers/' + element).once('value', data2 => {
