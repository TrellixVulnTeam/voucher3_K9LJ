...
let HomePage = class HomePage {
    ...
    ionViewDidEnter() {
        ...
        firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('users/' + this.user).once('value', data2 => {
            if (data2.exists()) {
                this.influencer = data2.val();
                // console.log(this.influencer)
            }
            else {
                firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('users/' + "Ypgf8VDQJrRhsrP7RREb3n321sf1").once('value', data3 => {
                    this.influencer = data3.val();
                });
            }
        });
        firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('vouchers/' + this.id).once('value', data => {
            if (data.exists()) {
                firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('vouchers/' + this.id + '/share/' + this.user).transaction(a => {
                    return a ? a + 1 : a = 1;
                }).then(c => {
            ...
            firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('vendors/' + this.voucher['by']).once('value', data => {
    ...
    fpx() {
        ...
        let keyer = firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('orders_pending').push({
        ...
        if (this.email && this.name) {
            console.log(body);
            this.http.post('https://us-central1-newvsnap.cloudfunctions.net/vsnapsql/revenuePayment', body).subscribe(a => {
    ...
    fpx_old() {
        ...
        localforage__WEBPACK_IMPORTED_MODULE_7__["setItem"]('vsnap_userinfo', temp);
        let key = firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('orders_pending').push({
            variations: (this.voucher.variations || []).reduce((a, b, i) => a.concat(b['selections'][this.var_select[i]]), []),
            address: this.voucher.address || false,
            date: firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database.ServerValue.TIMESTAMP,
        ...
        let body = {
            ...
            amount: this.proper2(((this.voucher.price_now + this.countvar())) || 0) * this.qty * 100,
            qty: this.qty || 1,
            description: 'Purchase ' + this.items.name + ' x' + this.qty || false,
            email: this.email,
            name: this.name,
            order: key,
            address: (this.address + ', ' + this.postcode + ', ' + this.city) || '',
    ...
    free() {
        ...
        if (this.email && this.name) {
            let ids = firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('orders').push({
                variations: temp,
                address: this.voucher.address || false,
                date: firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database.ServerValue.TIMESTAMP,
                by: this.voucher.by || "",
            ...
            firebase_app__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('orders/' + ids).update({
                id: ids,
            }).then(a => {
                // console.log(2);
