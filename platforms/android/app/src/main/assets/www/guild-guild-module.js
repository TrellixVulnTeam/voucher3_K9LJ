...
let GuildPage = class GuildPage {
    ...
    ionViewWillEnter() {
        ...
        firebase_app__WEBPACK_IMPORTED_MODULE_6__["default"].database().ref('vouchers/').once('value', data => {
            this.voucher = Object.values(data.val()).filter(a => ((a['price_vsnap'] || 0) / (a['price_now'] || 1) >= 0.1) && a['tag'] != "CSR2021" && a['status'] == true);
        ...
        firebase_app__WEBPACK_IMPORTED_MODULE_6__["default"].database().ref('guild/' + this.guild).once('value', data => {
            if (data.exists()) {
                ...
                firebase_app__WEBPACK_IMPORTED_MODULE_6__["default"].database().ref('users/' + this.user).once('value', data2 => {
                    if (data2.exists()) {
                        this.influencer = data2.val();
                        console.log(this.influencer);
                    }
                    else {
                        firebase_app__WEBPACK_IMPORTED_MODULE_6__["default"].database().ref('users/' + Object.values(this.guildinfo['core']).filter(a => a['position'] == 'leader')[0]['id']).once('value', data3 => {
